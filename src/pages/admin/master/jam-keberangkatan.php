            ...
            <div class="xl:col-span-2">
                <div class="flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md overflow-hidden">
                    <div class="relative bg-clip-border rounded-xl overflow-hidden bg-transparent text-gray-700 shadow-none m-0 flex items-center justify-between p-6">
                        <div>
                            <h6 class="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900 mb-1">Terdapat <span x-text="properties.data.list_jam_keberangkatan.length"></span> Jam Keberangkatan</h6>
                        </div>
                    </div>

                    <template x-if="properties.data.list_jam_keberangkatan.length > 0">
                        <div class="p-6 px-0 pt-0 pb-0">
                            <table class="w-full min-w-[640px] table-auto">
                                ...
                                <tbody>
                                <template x-for="(entity, index) in properties.data.list_jam_keberangkatan" :key="index">
                                    <tr>
                                        ...
                                        <td class="py-3 px-5 border-b border-gray-200">
                                            <p class="block antialiased font-sans text-xs font-medium text-gray-900 font-bold" x-text="entity.jam + ' WIB (' + entity.alias.toUpperCase() + ')'"></p>
                                        </td>
                                        <td class="py-3 px-5 border-b border-gray-200 flex justify-end gap-2">
                                            <button class="bg-orange-400 text-white rounded px-4 py-1 font-sans center" @click="editData(entity)">Edit</button>
                                            <button class="bg-red-500 text-white rounded px-4 py-1 font-sans center" @click="hapusData(entity)">Hapus</button>
                                        </td>
                                    </tr>
                                </template>
                                </tbody>
                ...
                <form @submit.prevent="simpanData">
                    <div class="flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md p-6">
                        <div class="relative bg-clip-border rounded-xl overflow-hidden bg-transparent text-gray-700 shadow-none m-0">
                            <h6 class="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900 mb-2"><span x-text="properties.sites.button_title"></span> Jam Keberangkatan</h6>
                        </div>
                        <div class="w-full min-w-[200px] mb-4">
                            <label for="" class="font-sans text-base text-gray-500 mb-2 block">Jam Keberangkatan (format 24jam)<small x-text="properties.sites.query_title"></small></label>
                            <input type="time" x-model="properties.form.jam" class="w-full bg-transparent text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400" autofocus>
                        </div>
                        <div class="w-full min-w-[200px] mb-4">
                            <label for="" class="font-sans text-base text-gray-500 mb-2 block">Alias (cth: PAGI, SIANG, MALAM)</label>
                            <input type="text" x-model="properties.form.alias" class="w-full bg-transparent text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400">
                        </div>
    ...
    document.addEventListener('alpine:init', () => {
        ...
        const actions = {
            ...
            "editData": function (entity) {
                ...
                this.properties.form.jam = entity.jam;
                this.properties.form.alias = entity.alias;
            ...
            "simpanData": function () {
                ...
                this.postData(
                    '/api/admin/jam-keberangkatan/simpan',
                    this.createFormData({
                        'id': this.properties.form.id,
                        'jam': this.properties.form.jam,
                        'alias': this.properties.form.alias,
                    }),
                    function (response) {
                        elem.disabled = false;
                        ...
                        if (alpineObj.properties.form.id < 0) {
                            alpineObj.properties.data.list_jam_keberangkatan.push({'id': response.data.data.id, 'jam': response.data.data.jam, 'alias': response.data.data.alias});

                            alpineObj.properties.form.jam = '';
                            alpineObj.properties.form.alias = '';
                            alpineObj.properties.form.id = -1;

                            return;
                        }

                        let index = alpineObj.properties.data.list_jam_keberangkatan.findIndex(item => item.id == response.data.data.id);
                        alpineObj.properties.data.list_jam_keberangkatan[index].jam = response.data.data.jam;
                        alpineObj.properties.data.list_jam_keberangkatan[index].alias = response.data.data.alias;

                        alpineObj.addNormalMessage('form_response', `Berhasil! Jam Keberangkatan (${response.data.data.jam} / ${response.data.data.alias}) telah disimpan`);
                    },
                    function (err) {
            ...
            "hapusData": function (entity) {
                ...
                let alpineObj = this;
                this.postData(
                    '/api/admin/jam-keberangkatan/hapus',
                    this.createFormData({
                        'id': entity.id,
                    }),
                    function (response) {
                        let index = alpineObj.properties.data.list_jam_keberangkatan.findIndex(item => item.id == response.data.data.deleted_id);
                        alpineObj.properties.data.list_jam_keberangkatan.splice(index, 1);
