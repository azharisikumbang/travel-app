...
<main x-data="container">
    <nav class="block w-full max-w-full bg-transparent text-white shadow-none transition-all px-0 py-1 border-b-2">
        <div class="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
            <h2 class="block antialiased tracking-normal font-sans text-2xl font-semibold leading-relaxed text-gray-900">Master - Rute Perjalanan</h2>
            ...
            <div class="xl:col-span-2">
                <div class="flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md overflow-hidden">
                    <div class="relative bg-clip-border rounded-xl overflow-hidden bg-transparent text-gray-700 shadow-none m-0 flex items-center justify-between p-6">
                        <div>
                            <h6 class="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900 mb-1">Terdapat <span x-text="properties.data.list_rute_perjalanan.length"></span> Rute Perjalanan</h6>
                        </div>
                    </div>

                    <template x-if="properties.data.list_rute_perjalanan.length > 0">
                            ...
                            <table class="w-full min-w-[640px] table-auto">
                                ...
                                <tr>
                                    ...
                                    <th class="border-b border-gray-200 py-3 px-6 text-left">
                                        <p class="block antialiased font-sans text-[11px] font-medium uppercase text-gray-400">Asal</p>
                                    </th>
                                    <th class="border-b border-gray-200 py-3 px-6 text-left">
                                        <p class="block antialiased font-sans text-[11px] font-medium uppercase text-gray-400">Tujuan</p>
                                    </th>
                                    <th class="border-b border-gray-20 py-3 px-6 text-left"></th>
                                </tr>
                                </thead>
                                <tbody>
                                <template x-for="(daerah, index) in properties.data.list_rute_perjalanan" :key="index">
                                    <tr>
                                        ...
                                        <td class="py-3 px-5 border-b border-gray-200">
                                            <p class="block antialiased font-sans text-xs font-medium text-gray-900 font-bold" x-text="daerah.asal.nama_kota"></p>
                                        </td>
                                        <td class="py-3 px-5 border-b border-gray-200">
                                            <p class="block antialiased font-sans text-xs font-medium text-gray-900 font-bold" x-text="daerah.tujuan.nama_kota"></p>
                                        </td>
                ...
                <form @submit.prevent="simpanData">
                    <div class="flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md p-6">
                        <div class="relative bg-clip-border rounded-xl overflow-hidden bg-transparent text-gray-700 shadow-none m-0">
                            <h6 class="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900 mb-2"><span x-text="properties.sites.button_title"></span> Wilayah Operasional</h6>
                        </div>
                        <div class="w-full min-w-[200px] mb-4">
                            <label for="" class="font-sans text-base text-gray-500 mb-2 block">Rute Asal<small x-text="properties.sites.query_title"></small></label>
                            <select x-model="properties.form.asal" class="w-full bg-transparent text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400">
                                <option value="-1">-- Piliha Kota --</option>
                                <template x-for="(daerah, index) in properties.data.list_daerah_operasional" :key="index">
                                    <option :value="daerah.id" x-text="daerah.nama_kota"></option>
                                </template>
                            </select>
                        </div>
                        <div class="w-full min-w-[200px] mb-4">
                            <label for="" class="font-sans text-base text-gray-500 mb-2 block">Rute Asal<small x-text="properties.sites.query_title"></small></label>
                            <select x-model="properties.form.tujuan" class="w-full bg-transparent text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400">
                                <option value="-1">-- Piliha Kota --</option>
                                <template x-for="(daerah, index) in properties.data.list_daerah_operasional" :key="index">
                                    <option :value="daerah.id" x-text="daerah.nama_kota"></option>
                                </template>
    ...
    document.addEventListener('alpine:init', () => {
        ...
        const actions = {
            ...
            "editData": function (daerah) {
                this.properties.form.asal = daerah.asal.id;
                this.properties.form.tujuan = daerah.tujuan.id;
                this.properties.form.id = daerah.id;

                this.properties.sites.query_title = `(dipilih: ${daerah.asal.nama_kota})`;
                ...
                this.postData(
                    ...
                    function (response) {
                        ...
                        if (alpineObj.properties.form.id < 0) {
                            alpineObj.properties.data.list_rute_perjalanan.push(response.data.data);

                            alpineObj.properties.form.asal = -1;
                            alpineObj.properties.form.tujuan = -1;
                            alpineObj.properties.form.id = -1;

                            alpineObj.addNormalMessage('form_response', `Berhasil! Rute ${response.data.data.asal.nama_kota} - ${response.data.data.tujuan.nama_kota} telah disimpan.`);

                            return;
                        }

                        let index = alpineObj.properties.data.list_rute_perjalanan.findIndex(item => item.id == response.data.data.id);
                        alpineObj.properties.data.list_rute_perjalanan[index].asal = response.data.data.asal;
                        alpineObj.properties.data.list_rute_perjalanan[index].tujuan = response.data.data.tujuan;

                        alpineObj.addNormalMessage('form_response', `Berhasil! Rute perjalanan ${response.data.data.asal.nama_kota} - ${response.data.data.tujuan.nama_kota} telah diperbaharui.`);
                    },
                    function (err) {
            ...
            "hapusData": function (daerah) {
                if(!confirm('Anda yakin ingin menghapus rute ?')) return;

                this.clearMassage();

                let alpineObj = this;
                this.postData(
                    '/api/admin/rute/hapus',
                    this.createFormData({
                        'id': daerah.id,
                    }),
                    function (response) {
                        let index = alpineObj.properties.data.list_rute_perjalanan.findIndex(item => item.id == response.data.data.deleted_id);
                        alpineObj.properties.data.list_rute_perjalanan.splice(index, 1);

                        alpineObj.addNormalMessage('form_response', 'Berhasil! Rute perjalanan telah dihapus.');
                    },
                    function (err) {
        ...
        Alpine.data('container',
            () => ({
                ...
                "properties": {
                    ...
                    "data": {
                        "list_rute_perjalanan": JSON.parse('<?= json_encode(array_map(fn ($item) => $item->toArray(), $listRute)) ?>'),
