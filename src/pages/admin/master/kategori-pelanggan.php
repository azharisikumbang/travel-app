<?php $listKategoriPelanggan = app()->getManager()->getService('KategoriPelangganService')->listKategoriPelanggan();?>
<main x-data="container">
    <nav class="block w-full max-w-full bg-transparent text-white shadow-none transition-all px-0 py-1 border-b-2">
        <div class="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
            <h2 class="block antialiased tracking-normal font-sans text-2xl font-semibold leading-relaxed text-gray-900">Master - Kategori Pelanggan</h2>
            ...
            <div class="xl:col-span-2">
                <div class="flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md overflow-hidden">
                    <div class="relative bg-clip-border rounded-xl overflow-hidden bg-transparent text-gray-700 shadow-none m-0 flex items-center justify-between p-6">
                        <div>
                            <h6 class="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900 mb-1">Terdapat <span x-text="properties.data.list_kategori_pelanggan.length"></span> Kategori Pelanggan</h6>
                        </div>
                    </div>

                    <template x-if="properties.data.list_kategori_pelanggan.length > 0">
                        <div class="p-6 px-0 pt-0 pb-0">
                            <table class="w-full min-w-[640px] table-auto">
                                ...
                                <tbody>
                                <template x-for="(entity, index) in properties.data.list_kategori_pelanggan" :key="index">
                                    <tr>
                                        ...
                                        <td class="py-3 px-5 border-b border-gray-200">
                                            <p class="block antialiased font-sans text-xs font-medium text-gray-900 font-bold" x-text="entity.kategori"></p>
                                        </td>
                ...
                <form @submit.prevent="simpanData">
                    <div class="flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md p-6">
                        <div class="relative bg-clip-border rounded-xl overflow-hidden bg-transparent text-gray-700 shadow-none m-0">
                            <h6 class="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-gray-900 mb-2"><span x-text="properties.sites.button_title"></span> Kategori Pelanggan</h6>
                        </div>
                        <div class="w-full min-w-[200px] mb-4">
                            <label for="" class="font-sans text-base text-gray-500 mb-2 block">Nama Kategori <small x-text="properties.sites.query_title"></small></label>
                            <input type="text" x-model="properties.form.kategori" class="w-full bg-transparent text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400" autofocus>
                        </div>
    ...
    document.addEventListener('alpine:init', () => {
        ...
        const actions = {
            ...
            "editData": function (entity) {
                this.properties.form.kategori = entity.kategori;
                this.properties.form.id = entity.id;

                this.properties.sites.query_title = `(dipilih: ${entity.kategori})`;
            ...
            "simpanData": function () {
                ...
                this.postData(
                    '/api/admin/kategori-pelanggan/simpan',
                    this.createFormData({
                    ...
                    function (response) {
                        ...
                        if (alpineObj.properties.form.id < 0) {
                            alpineObj.properties.data.list_kategori_pelanggan.push({'id': response.data.data.id, 'kategori': response.data.data.kategori});

                            alpineObj.properties.form.kategori = null;
                            alpineObj.properties.form.id = -1;

                            return;
                        }

                        let index = alpineObj.properties.data.list_kategori_pelanggan.findIndex(item => item.id == response.data.data.id);
                        alpineObj.properties.data.list_kategori_pelanggan[index].kategori = response.data.data.kategori;

                        alpineObj.addNormalMessage('form_response', `Berhasil! Kategori ${response.data.data.kategori} telah disimpan`);
                    },
                    function (err) {
            ...
            "hapusData": function (entity) {
                ...
                let alpineObj = this;
                this.postData(
                    '/api/admin/kategori-pelanggan/hapus',
                    this.createFormData({
                        'id': entity.id,
                    }),
                    function (response) {
                        let index = alpineObj.properties.data.list_kategori_pelanggan.findIndex(item => item.id == response.data.data.deleted_id);
                        alpineObj.properties.data.list_kategori_pelanggan.splice(index, 1);
