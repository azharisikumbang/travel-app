                        ...
                        <div class="flex justify-between w-full items-center">
                            <div>
                                <h6
                                    ...
                                    Menampilkan Tarif Per Kategori Penumpang</h6>
                            </div>
                            <div>
                                <span>Tampilkan : </span>
                                <select x-model="properties.form.filter.kategori"
                                    class="cursor-pointer text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400">
                                    <option value="-1">-- Semua Ketegori --</option>
                                    <template x-for="(entity, index) in properties.data.list_kategori_penumpang"
                                        :key="index">
                    ...
                    <template x-if="properties.data.list_tarif.length > 0">
                        <div class="p-6 px-0 pt-0 pb-0 table-wrp block max-h-screen">
                            <table class="w-full min-w-[640px] table-auto">
                                ...
                                <tbody class="overflow-y-auto scrollbar">
                                        ...
                                        <tr class="text-center" x-show="isFiltered(entity.kategori.id)">
                                            <td class="py-3 px-5 border-b border-gray-200">
                                                <p class="block antialiased font-sans text-xs font-medium text-gray-900 font-bold"
                                                    x-text="entity.kategori.kategori"></p>
                                            </td>
                                            <td class="py-3 px-5 border-b border-gray-200 text-left">
                                                <p class="block antialiased font-sans text-xs font-medium text-gray-900 font-bold"
                                                    x-text="entity.rute.asal.nama_kota + ' - ' + entity.rute.tujuan.nama_kota">
                                                </p>
                                            </td>
                                            <td class="py-3 px-5 border-b border-gray-200">
                                                <p class="block antialiased font-sans text-xs font-medium text-gray-900 font-bold"
                                                    x-text="rupiah(entity.tarif)"></p>
                                            </td>
                                            <td class="py-3 px-5 border-b border-gray-200 flex justify-end gap-2">
                                                <button
                                                    class="bg-yellow-500 text-white rounded px-4 py-1 font-sans center hover:bg-yellow-600 trigger-edit"
                                                    @click="editData(entity)">Edit</button>
                                                <button
                                                    class="bg-red-500 text-white rounded px-4 py-1 font-sans center hover:bg-red-600"
                                                    @click="hapusData(entity)">Hapus</button>
                                            </td>
                                        </tr>
                                    </template>
                                </tbody>
                    ...
                    <div class="flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md p-6">
                        <div class="relative bg-clip-border rounded-xl overflow-hidden text-gray-700 shadow-none m-0">
                            <h6
                                ...
                                <span x-text="properties.sites.button_title"></span> Rute Tiket
                            </h6>
                        </div>
                        <div class="w-full min-w-[200px] mb-4">
                            <label for="" class="font-sans text-base text-gray-500 mb-2 block">Kategori
                                Penumpang</label>
                            <select x-model="properties.form.kategori"
                                class="w-full cursor-pointer text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400"
                                required>
                        ...
                        <div class="w-full min-w-[200px] mb-4">
                            ...
                            <select x-model="properties.form.rute"
                                class="w-full cursor-pointer text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400"
                                required>
                                <option value="-1">-- Pilih Rute --</option>
                                <template x-for="(entity, index) in properties.data.list_rute" :key="index">
                                    <option :value="entity.id"
                                        x-text="entity.asal.nama_kota + ' - ' + entity.tujuan.nama_kota"></option>
                                </template>
                            </select>
                        </div>
                        <div class="w-full min-w-[200px] mb-4">
                            <label for="" class="font-sans text-base text-gray-500 mb-2 block">Nominal Tarif
                                (Rupiah)</label>
                            <input type="number" x-model="properties.form.tarif"
                                class="w-full text-gray-700 font-sans font-normal outline outline-0 border-2 text-sm px-3 py-3 rounded-md border-gray-200 focus:border-gray-400">
                        </div>
                        <div class="w-full min-w-[200px]">
                            <button @click="simpanData" type="submit"
                                class="bg-green-500 w-full text-white rounded py-4 font-sans center hover:bg-green-600"
